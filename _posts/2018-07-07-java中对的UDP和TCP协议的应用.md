...
* A:数据包和发送对象介绍:
  * `DatagramPacket`数据包的作用就如同是“集装箱”，可以将发送端或者接收端的数据封装起来。然而运输货物只有“集装箱”是不够的，还需要有码头。
  * 在程序中需要实现通信只有`DatagramPacket`数据包也同样不行，为此JDK中提供的一个`DatagramSocket`类。
...
### 14. TCP的服务器程序accept方法

* A:TCP的服务器程序`accept`方法
...
转载请注明原地址：[wonderheng的博客](http://www.wonderheng.top) » [点击阅读原文](http://www.wonderheng.top/2018/07/java中对的UDP和TCP协议的应用),谢谢！